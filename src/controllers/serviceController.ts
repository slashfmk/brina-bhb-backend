...
export const createService = async (
  ...
  if (!name || !description || !price || !points)
    return next({ message: "Please fill out all the fields" });
  ...
  db.query(
    ...
    (error: QueryError, result: RowDataPacket[]) => {
      ...
      if (result.length > 0)
        return next({ message: `${name} service already exists!` });
      ...
      db.query(
        ...
        (error2: QueryError, result2: RowDataPacket[]) => {
          if (error2) return next({ message: error2 });
          res.status(200).json({
            message: `Service ${name} has been created successfully!`,
          });
          next();
        }
      );
...
export const deleteService = async (
  ...
  if (!serviceId)
    return next({ message: "Please send the service id to delete!" });
  ...
  db.query(
    ...
    (error: QueryError, result: RowDataPacket[]) => {
      ...
      if (result.length <= 0)
        return next({ message: `No service exists for the provided id!` });
      ...
      db.query(
        ...
        (error2: QueryError, result2: RowDataPacket[]) => {
          if (error2) return next({ message: error2 });
          res.status(200).json({
            message: `Service ${result[0].name} has been deleted successfully!`,
          });
          next();
        }
      );
