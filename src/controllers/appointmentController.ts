...
export const addAppointment = async (
  ...
  if (!date || !time || !note)
    return next({ message: "Fill out all the fields please!" });
  //@ts-ignore
...
export const deleteAppointment = async (
  ...
  db.query(
    ...
    (error: QueryError, result: RowDataPacket[]) => {
      if (result.length < 1)
        return next({ message: "There is no such appointment to delete!" });
...
export const toggleAppointmentStatus = async (
  ...
  if (Number.isNaN(appointmentId))
    return next({ message: `Bad id format provided! must be a number` });
  ...
  db.query(
    ...
    (error: QueryError, result: RowDataPacket[]) => {
      if (error) return next({ message: error });
      if (result.length < 1)
        return next({ message: `There is no such appointment!` });
      ...
      db.query(
        ...
        (error2: QueryError, result2: RowDataPacket[]) => {
          ...
          res.status(200).json({
            message: `Appointment successfully updated to ${
              updatedStatus ? "Done" : "Not Done"
            }`,
          });
...
export const updateAppointment = (
  ...
  db.query(
    ...
    (error: QueryError, result: RowDataPacket[]) => {
      if (error) return next({ message: error });
      if (result.length < 1)
        return next({ message: `No appointment found for id ${appId}` });
      ...
      db.query(
        ...
        (error2: QueryError, result2: RowDataPacket[]) => {
          ...
          if (result2.length < 1)
            return next({
              message: "The appointment, you are trying to edit is not yours!!",
            });
            ...
            db.query(
              ...
              (error3: QueryError, result3: RowDataPacket[]) => {
                ...
                return res.status(200).json({
                  message: "Appointment successfully updated with no new note!",
                });
              }
            );
          } else {
